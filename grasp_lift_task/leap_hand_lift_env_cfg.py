...
@configclass
class LeapHandGraspLiftEnvCfg(DirectRLEnvCfg):
    """Config for LEAP Hand grasp-and-lift (direct RL env)."""
    ...
    # robot: floating base near the object with contact sensing enabled
    _rot_x_pi = (0.0, 1.0, 0.0, 0.0)
    robot_cfg: ArticulationCfg = LEAP_HAND_CFG.replace(
        prim_path="/World/envs/env_.*/Robot",
        spawn=LEAP_HAND_CFG.spawn.replace(
            activate_contact_sensors=True,
            rigid_props=LEAP_HAND_CFG.spawn.rigid_props.replace(disable_gravity=False),
            articulation_props=LEAP_HAND_CFG.spawn.articulation_props.replace(fix_root_link=False),
        ),
    ...
    # LEAP hand USD link naming differs across assets.
    # In this repository's LEAP hand USD, the palm/root body is exposed as "base".
    # The environment will also fall back to best-effort matching if this exact name isn't found.
    palm_body_name: str = "base"

    # reward scales
    reach_dist_scale: float = 2.0
    contact_bonus: float = 2.0
    # "Contact" proxy for RViz/training without Isaac contact sensors (meters).
    ...
    fallen_z: float = -0.05
    success_lift_height: float = 0.10  # meters above initial

    # grasp confirmation + auto-lift
    grasp_confirm_steps: int = 5
    auto_lift_on_grasp: bool = True
